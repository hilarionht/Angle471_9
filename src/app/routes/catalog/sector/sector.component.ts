...
export class SectorComponent implements OnInit {
  ...
  editbyid(content, id: number) {
    ...
    if (id) {
      this.sectorService.findOne(id).subscribe((resp: any) =>
        {
  ...
  confirm(pdelete, id: any) {
    ...
    this.sector = new Sector(null, 0, null, null);
    if (id) {
      this.sectorService.findOne(id).subscribe((resp: any) => {this.sector = resp.data; });
    }
  ...
  save(content, form?: NgForm) {
    if (form.value.id !== '0') {
      ...
      this.sectorService.update(form.value.id, form.value)
      .subscribe(res => {
        this.setPage({offset: 0});
      });
    } else {
      form.value.name = form.value.name.toUpperCase();
      this.sectorService.save(form.value)
        .subscribe(resp => {
          this.setPage({offset: 0});
        });
    }
